...
def get_parser_args():
    ...
    parser_eval.add_argument('--task', type=str, default="zeroshot_classification", choices=["zeroshot_classification", "zeroshot_retrieval", "linear_probe", "captioning", "image_caption_selection", "auto"], help="Task to evaluate on. With --task=auto, the task is automatically inferred from the dataset.")
...
def main():
    ...
    if not hasattr(base, "which"):
        ...
        return
    if base.which == "eval":
        main_eval(base) # getting call here
...
def run(args):
    ...
    if args.skip_load:
        ...
        print(args.model_type, args.model, args.pretrained, args.model_cache_dir, args.device)
        # exit()
