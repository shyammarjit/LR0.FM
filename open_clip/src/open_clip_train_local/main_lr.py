...
import sys
sys.path.append("src")
# sys.path.append("src/open_clip_train_local")

import os
previous_dir = os.getcwd() # Folder A
sys.path.append(previous_dir)
from zero_shot import get_classes_prompts, create_dataset_zero_shot, zeroshot_classifier
...
from open_clip_train_local.train import train_one_epoch, train_SS_MS
from open_clip_train_local.data_local import get_data
...
def main(args):
    ...
    if args.model == "OpenCLIP-ViT-B/16":
        args.model = 'ViT-B-16'
    ...
    #################### VALIDATION 
    args.dataset_dir = '/data/priyank/synthetic/food-101/'
    ...
    test = create_dataset_zero_shot(args.dataset, dtd_split=1, low_resolution=args.low_resolution, org_resolution=224, model_name='openclip', root=args.dataset_dir)
    loader = torch.utils.data.DataLoader(test, batch_size=400, num_workers=args.workers)
...
def copy_codebase(args):
    ...
    os.system(f"rm -rf {new_code_path}")
