...
def create_dataset(dataset, config, min_scale=0.5, low_resolution=384, original_resolution=384, root=None):
...
def create_dataset_zero_shot(dataset, config, min_scale=0.5, low_resolution=128, original_resolution=384, dtd_split=1, root=None):
    normalize = transforms.Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711))      
    if low_resolution != original_resolution:  
        transform_test = transforms.Compose([
            transforms.Resize((low_resolution, low_resolution),interpolation=InterpolationMode.BICUBIC),
