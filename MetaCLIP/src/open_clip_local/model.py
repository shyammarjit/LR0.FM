...
class Transformer_LR(Transformer):
    def __init__(self, width: int, layers: int, heads: int,  mlp_ratio: float = 4.0, act_layer: Callable = nn.GELU, num_patches=0):
        super().__init__(width=width, layers=layers, heads=heads, mlp_ratio=mlp_ratio, act_layer=act_layer)
        self.lock_existing_Layers()

        self.blocks_spatial_tokens = nn.ParameterList([
                nn.Parameter(torch.zeros(num_patches, 1, width))
    ...
    def add_spatial_tokens(self, x, i):
        ...
        x = x + spatial_tokens
        x = torch.cat((cltokens.unsqueeze(0), x), dim=0)
        return x 
...
class VisualTransformer_LR(VisualTransformer):
    def __init__(self, lock=True, layers=-1, width=0, heads=0, mlp_ratio=10000, act_layer= nn.GELU, **kwargs):
        super().__init__(layers=layers, width=width, heads=heads, mlp_ratio=mlp_ratio, act_layer=act_layer, **kwargs)
        ...
        self.transformer = Transformer_LR(width, layers, heads, mlp_ratio, act_layer=act_layer, num_patches=self.grid_size[0] * self.grid_size[1])
