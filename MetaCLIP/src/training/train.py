...
from .zero_shot_local import zero_shot_eval

from torchvision.utils import save_image
from einops import rearrange, repeat

from training.metaclip_wds import normalize
...
# MULTI SCALE + SS
def train_SS_MS(model, data, epoch, epoch_step, optimizer, scaler, scheduler, args, tb_writer=None):
    device = torch.device(args.device)
    autocast = torch.cuda.amp.autocast if args.precision == 'amp' else suppress
    model.train()
    ...
    for i, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            image_features_HR_Truth, _, logit_scale = model(IMG_HR, None, use_spatial_tokens=False)
        ...
        with autocast():
            ...
            total_loss_hr = loss(image_features_HR_Truth, image_features_HR, logit_scale)
            ...
            if N_LR == 1:
                ...
                total_loss_lr = 0
                for i in range(N_LR):
                    loss_lr = loss(image_features_HR_Truth, image_features_LR[:,i], logit_scale)
