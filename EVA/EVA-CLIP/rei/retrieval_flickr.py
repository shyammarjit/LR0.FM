...
model, a, b = create_model_and_transforms(args.backbone, pretrained, force_custom_clip=True)
model = model.cuda()
tokenizer = get_tokenizer(args.backbone) # get the tokenizer
...
dataset = flickr30k(transform, coco_root, coco_ann_file, target_transform=lambda texts: tokenizer(texts))
