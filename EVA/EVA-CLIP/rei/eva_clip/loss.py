...
class feat_lr(ClipLoss):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        # self.MSE = MSE()
        self.MSE = cosine_dist
    ...
    def forward(self, image_features, text_features, logit_scale=1.):
        ...
        if self.world_size > 1:
            ...
            total_loss = self.MSE(all_image_features, all_text_features)
        else:
            total_loss = self.MSE(image_features, text_features)
            
        # calculated ground-truth and cache if enabled
        acc = None
        i2t_acc = 1 - total_loss
        t2i_acc = 1 - total_loss
        acc = {"i2t": i2t_acc, "t2i": t2i_acc}
        return total_loss, acc
