...
class TripletLoss_backup(object):
    """
...
class TripletLoss(TripletLoss_backup, RankingLoss):
    def __call__(self, global_feat, labels, normalize_feature=False):
        if normalize_feature:
            global_feat = normalize(global_feat, axis=-1)
        dist_mat = euclidean_dist(global_feat, global_feat)
        
        mat_sim = self._label2similarity(labels, labels)
        dist_ap, dist_an = self._batch_hard(dist_mat, mat_sim.float(), more_similar='smaller')
...
class TripletLoss_w_anchors(RankingLoss):
